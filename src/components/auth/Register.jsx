    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100 w-full h-screen md:p-4">
      <div className="bg-white shadow-lg rounded-md flex w-full h-full overflow-hidden relative max-w-4xl mx-auto">
        ...
        <div className="w-full md:w-1/2 h-full p-6 md:p-8 flex flex-col justify-center items-center relative">
          ...
          {/* Scrollable Form */}
          <div className="w-full max-w-sm overflow-y-auto" style={{ maxHeight: "70vh" }}>
            <form onSubmit={handleSubmit}>

              {/* User Type Buttons */}
              <div className="flex mb-4 w-full justify-between gap-2">
                <Button type={formData.userType === "STUDENT" ? "primary" : "default"} onClick={() => handleToggleUserType("STUDENT")} className="w-full">
                  Student
                </Button>
                <Button type={formData.userType === "ADMIN" ? "primary" : "default"} onClick={() => handleToggleUserType("ADMIN")} className="w-full">
                  Admin
                </Button>
                <Button type={formData.userType === "SUPER_ADMIN" ? "primary" : "default"} onClick={() => handleToggleUserType("SUPER_ADMIN")} className="w-full">
                  Super Admin
                </Button>
              </div>

              {/* Common Fields */}
              <Input name="firstName" placeholder="First Name" size="large" className="mb-3 w-full" onChange={handleChange} required />
              <Input name="lastName" placeholder="Last Name" size="large" className="mb-3 w-full" onChange={handleChange} required />
              <Input name="email" placeholder="Email" size="large" className="mb-3 w-full" onChange={handleChange} required />
              <Input.Password name="password" placeholder="Password" size="large" className="mb-3 w-full" onChange={handleChange} required />
              <Input name="country" placeholder="Country" size="large" className="mb-3 w-full" onChange={handleChange} required />
              <Input name="phone" placeholder="Phone Number" size="large" className="mb-3 w-full" onChange={handleChange} required />
              ...
              {formData.userType === "STUDENT" && (
                <>
                  ...
                  <Input name="whatsappNumber" placeholder="WhatsApp Number" size="large" className="mb-3 w-full" onChange={handleChange} required />
                  <Input name="address" placeholder="Address" size="large" className="mb-3 w-full" onChange={handleChange} required />
                  <Input name="district" placeholder="District" size="large" className="mb-3 w-full" onChange={handleChange} required />
                  <Input name="state" placeholder="State" size="large" className="mb-3 w-full" onChange={handleChange} required />
...
/>
              ...
              {formData.userType === "SUPER_ADMIN" && (
                <>
                  <Input
                    ...
                    required
                  />
                  <Input name="department" placeholder="Department" size="large" className="mb-3 w-full" onChange={handleChange} required />
                  <Input name="permissionLevel" placeholder="Permission Level" size="large" className="mb-3 w-full" onChange={handleChange} required />
                </>
              )}
              ...
              <Link to="/" className="block">
                <Button type="default" block size="large"> Back to Home </Button>
              </Link>
